    ...
    switch(opcode) {
       case 0x88: {  // MOV REG8/MEM8, REG8: 0x88, MOD REG R/M, (DISP-LO), (DISP-HI)]
        ...
        case 0xA2: {  // MOV MEM8, AL: [0xA2, DISP-LO, DISP-HI]
            uint16_t addr = get_addr(DS_register, data[0] + (data[1] << 8));
            mem_write(addr, get_register_value(AL_register), 2);
            ret_val = 3;
        ...
        case 0xC7: {  // MOV MEM16, IMMED16: [0xC7, MOD 000 R/M, (DISP-LO), (DISP-HI), DATA-LO, DATA-HI]
            if(get_register_field(data[0]) == 0) {
                ...
                operands_t operands = decode_operands(opcode & 0xFD, data); // Treat R/M field as destination
                // uint8_t mod_field = get_mode_field(data[0]);
...
uint8_t shift_instr(uint8_t opcode, uint8_t *data) {
    ...
    switch(opcode) {
        ...
        case 0xD1: {
            // 16-bit operations
            switch(get_register_field(data[0])) {
                case 0: // ROL REG16/MEM16, 1: [0xD1, MOD 000 R/M, DISP-LO, DISP-HI]
                    printf("ERROR: unimplemented ROL REG16/MEM16, 1 operation\n");
                    REGS->invalid_operations ++;
                    break;
                case 1: // ROR REG16/MEM16, 1: [0xD1, MOD 001 R/M, DISP-LO, DISP-HI]
                    printf("ERROR: unimplemented ROR REG16/MEM16, 1 operation\n");
                    REGS->invalid_operations ++;
                    break;
                case 2: // RCL REG16/MEM16, 1: [0xD1, MOD 010 R/M, DISP-LO, DISP-HI]
                    printf("ERROR: unimplemented RCL REG16/MEM16, 1 operation\n");
                    REGS->invalid_operations ++;
                    break;
                case 3: // RCR REG16/MEM16, 1: [0xD1, MOD 011 R/M, DISP-LO, DISP-HI]
                    printf("ERROR: unimplemented RCR REG16/MEM16, 1 operation\n");
                    REGS->invalid_operations ++;
                    break;
                case 4: // SAL/SHL REG16/MEM16, 1: [0xD1, MOD 100 R/M, DISP-LO, DISP-HI]
                    res_val = operands.dst_val << 1;
                    if(operands.dst_type == 0) {    // Register mode
                        set_register_value(operands.dst.register_name, res_val);
                    } else {    // Memory mode
                    ...
                    break;
                case 5: // SHR REG16/MEM16, 1: [0xD1, MOD 101 R/M, DISP-LO, DISP-HI]
                    printf("ERROR: unimplemented SHR REG16/MEM16, 1 operation\n");
                    REGS->invalid_operations ++;
                    break;
                case 6: // INVALID_INSTRUCTION
                    REGS->invalid_operations ++;
                    printf("Invalid instruction: 0x%02X, REG field 110\n", opcode);
                    break;
                case 7: // SAR REG16/MEM16, 1: [0xD1, MOD 111 R/M, DISP-LO, DISP-HI]
                    printf("ERROR: unimplemented SAR REG16/MEM16, 1 operation\n");
                    REGS->invalid_operations ++;
                    break;
            }
            break;
        ...
        case 0xD2: {
            ...
            switch(get_register_field(data[0])) {
                ...
                case 5: // SHR REG8/MEM8, CL: [0xD2, MOD 101 R/M, DISP-LO, DISP-HI]
                    operands.dst_val = operands.src_val;   // dst_val isn't unused because the REG field specifies operation type
                    operands.src_val = get_register_value(CL_register);
...
uint8_t div_instr(uint8_t opcode, uint8_t *data) {
    ...
    ret_val += operands.num_bytes;
    switch(opcode) {
        case 0xF7: {    // DIV REG16/MEM16: [0xF7, MOD110 R/M, (DISP-LO),(DISP-HI)]
...
uint8_t add_instr(uint8_t opcode, uint8_t *data) {
    ...
    switch(opcode) {
        case 0x81: {
            uint8_t reg_field = get_register_field(data[0]);
            if(reg_field == 0) {  // ADD REG16/MEM16,IMMED16: [0x81, MOD 000 R/M, (DISP-LO), (DISP-HI), DATA-LO, DATA-HI]
                if(operands.dst_type == 0) {    // Register mode
                    ret_val = 4;
        ...
        case 0x83: {
            operands = decode_operands(opcode & 0xC0, data);    // width 8 bit and dst in R/M field
            uint8_t reg_field = get_register_field(data[0]);
            if(reg_field == 0) { // ADD REG16/MEM16, IMMED8: [0x83, MOD 000 R/M, (DISP-LO),(DISP-HI), DATA-SX]
                if(operands.dst_type == 0) {    // Register mode
                    ret_val = 3;
                    operands.src_val = data[1];
                } else {
                    ret_val = 5;
                    operands.src_val = data[3];
                }
                res_val = operands.src_val + operands.dst_val;
                mylog("logs/main.log", "Instruction 0x%02X: ADD %s (0x%04X), immed8 (0x%04X); res = 0x%04X\n", opcode, operands.destination, operands.dst_val, operands.src_val, res_val);
                update_flags(operands.dst_val, operands.src_val, res_val, operands.width, ADD_OP);
                if(operands.dst_type == 0) {    // Register mode
                    set_register_value(operands.dst.register_name, res_val);
                } else {    // Memory mode
...
uint8_t or_instr(uint8_t opcode, uint8_t *data) {
    ...
    switch(opcode) {
        ...
        case 0x80: {  // OR REG8/MEM8, IMMED8: [0x80, MOD 001 R/M, (DISP-LO), (DISP-HI), DATA-8]
            ...
            if(operands.dst_type == 0) {    // Register mode
                ret_val = 3;
                res_val = operands.dst_val | data[1];
                src_val = data[1];
                set_register_value(operands.dst.register_name, res_val);
            } else {    // Memory mode
                ret_val = 5;
                res_val = operands.dst_val | data[3];
                src_val = data[3];
                mem_write(operands.dst.address, res_val, 1);
            }
            update_flags(operands.dst_val, operands.src_val, res_val, operands.width, LOGIC_OP);
            mylog("logs/main.log", "Instruction 0x%02X: OR %s (0x%04X), immed8 (0x%04X); result = 0x%04X\n", opcode, operands.destination, operands.dst_val, src_val, res_val);
            break;
        }
        default:
            REGS->invalid_operations ++;
...
uint8_t and_instr(uint8_t opcode, uint8_t *data) {
    ...
    switch(opcode) {
        ...
        case 0xF6: {
            if(0 == get_register_field(data[0])) {// TEST REG8/MEM8, IMMED8: [0xF6, MOD 000 R/M, (DISP-LO), (DISP-HI), DATA-8]
                operands = decode_operands(opcode & 0xFC, data);    // Get DST from the R/M field
                if(operands.dst_type == 0) {    // Register_mode
                    operands.src_val = data[1];
                    ret_val = 3;
                } else {
                ...
                REGS->invalid_operations ++;
                printf("Error: Invalid TEST instruction: 0x%02X: reg_field > 0\n", opcode);
            }
            break;
        }
        default:
...
uint8_t pop_reg_instr(uint8_t opcode, uint8_t *data) {
    ...
    switch(opcode) {
        case 0x1F: {  // POP DS
            pop_register(DS_register);
            break;
        }
        case 0x58: {  // POP AX
            pop_register(AX_register);
            break;
        }
        case 0x59: {  // POP CX
            pop_register(CX_register);
            break;
        }
        case 0x5A: {  // POP DX
            pop_register(DX_register);
            break;
        }
        case 0x5B: {  // POP BX
            pop_register(BX_register);
            break;
        }
        case 0x5C: {  // POP SP
            pop_register(SP_register);
            break;
        }
        case 0x5D: {  // POP BP
            pop_register(BP_register);
            break;
        }
        case 0x5E: {  // POP SI
            pop_register(SI_register);
            break;
        }
        case 0x5F: {  // POP DI
...
uint8_t push_reg_instr(uint8_t opcode, uint8_t *data) {
    ...
    switch(opcode) {
        ...
        case 0x53: {  // PUSH BX
            push_register(get_register_value(BX_register));
            mylog("logs/main.log", "Instruction 0x%02X: PUSH BX\n", opcode);
            break;
        }
        case 0x54: {  // PUSH SP
            push_register(get_register_value(SP_register));
            mylog("logs/main.log", "Instruction 0x%02X: PUSH SP\n", opcode);
            break;
        }
        case 0x55: {  // PUSH BP
            push_register(get_register_value(BP_register));
            mylog("logs/main.log", "Instruction 0x%02X: PUSH BP\n", opcode);
            break;
        }
        case 0x56: {  // PUSH SI
            push_register(get_register_value(SI_register));
            mylog("logs/main.log", "Instruction 0x%02X: PUSH SI\n", opcode);
            break;
        }
        case 0x57: {  // PUSH DI
